	...
	if (cell and cell.ability) then
		...
		if LBF then
			...
			if (not cell.button.lbf_nvcd) then
				...
				cell.button.lbf_nvcd.r, cell.button.lbf_nvcd.g, cell.button.lbf_nvcd.b, cell.button.lbf_nvcd.a =
					LBF:GetNormalVertexColor(cell.button)
			end

			if (frame.profile.coloredBorders == true) then
				LBF:SetNormalVertexColor(cell.button, c.r, c.g, c.b, c.a)
			else
				LBF:SetNormalVertexColor(cell.button, cell.button.lbf_nvcd.r, cell.button.lbf_nvcd.g, cell.button.lbf_nvcd.b, cell.button.lbf_nvcd.a)
			end
		else
			if (frame.profile.coloredBorders == true) then
				cell.button.normaltexture:SetVertexColor(c.r, c.g, c.b, c.a)
			else
				cell.button.normaltexture:SetVertexColor(cell.button.nvcd.r, cell.button.nvcd.g, cell.button.nvcd.b, cell.button.nvcd.a)
			end
		end
	else
...
function mod:CreateMergedCell(frame)
	...
	cell.button = cell.button or CreateFrame("Button", "Hermes-UI-" .. VIEW_NAME .. "MergedButton" .. tostring(UNIQUE_FRAME_COUNT), cell, "ActionButtonTemplate")
...
function mod:CreateCell()
	...
	-- CELL.BUTTON
	---------------------------------------------------------------
	cell.button = cell.button or CreateFrame("Button", "Hermes-UI-" .. VIEW_NAME .. "Button" .. tostring(UNIQUE_CELL_COUNT), cell, "ActionButtonTemplate")
	cell.button.icon = _G[cell.button:GetName() .. "Icon"]
	cell.button.flash = _G[cell.button:GetName() .. "Flash"]
	cell.button.count = _G[cell.button:GetName() .. "Count"]
	cell.button.name = _G[cell.button:GetName() .. "Name"]
...
function mod:UpdateCellStyleUnavailable(cell)
	...
	cell.button.count:SetText(nil)
	cell.button.icon:SetDesaturated(false)
	cell.button.icon:SetVertexColor(profile.colorU.r, profile.colorU.g, profile.colorU.b, profile.colorU.a)
...
function mod:UpdateCellStyleNoSenders(cell)
	...
	cell.button.count:SetText(nil)
	cell.button.icon:SetDesaturated(true)
	cell.button.icon:SetVertexColor(profile.colorNS.r, profile.colorNS.g, profile.colorNS.b, profile.colorNS.a)
		...
		color = {
			...
			args = {
				colorU = {
					...
					width = "normal",
					get = function(info)
						return profile.colorU.r, profile.colorU.g, profile.colorU.b, profile.colorU.a
					end,
				...
				colorNS = {
					...
					width = "normal",
					get = function(info)
						return profile.colorNS.r, profile.colorNS.g, profile.colorNS.b, profile.colorNS.a
					end,
					set = function(info, r, g, b, a)
						profile.colorNS.r = r
						profile.colorNS.g = g
			...
			args = {
				skin = {
					...
					set = function(info, value)
						...
						--skin it
						group:Skin(profile.lbf.skin, profile.lbf.gloss, profile.lbf.backdrop, profile.lbf.colors)
