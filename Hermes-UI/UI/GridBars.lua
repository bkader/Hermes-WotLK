-- requires LibHermesUICooldownBars-1.0
local LibBars = LibStub("LibHermesUICooldownBars-1.0")
if not LibBars then
	error("Required library LibHermesUICooldownBars-1.0 not found!")
end
...
local IMAGE_RESIZE = [[Interface\Addons\Hermes-UI\Textures\Resize.tga]]
...
function mod:RestoreFramePosition(frame) --sets the starting point for all window positions based on profile settings (if it exists)
	if (not frame.profile.x or not frame.profile.y) then
		...
		frame:ClearAllPoints()
		frame:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT", frame.profile.x, frame.profile.y)
...
function mod:InitializeCell(frame, cell, ability, ismerged)
	...
	cell:ClearAllPoints()
	cell.backgroundTexture:SetTexture(frame.profile.cellBGColor.r, frame.profile.cellBGColor.g, frame.profile.cellBGColor.b, frame.profile.cellBGColor.a)
...
local function cell_OnHide(self)
	self:StopMovingOrSizing()
end
...
function mod:CreateCell()
	...
	-- this is a precautionary measure to HOPEFULLY avoid any frames stuck to cursor
	-- issues while dragging if the frame gets hidden while dragging it (due to combat events)
	cell:SetScript("OnHide", cell_OnHide)
...
function mod:SetCellVisibility(cell)
	...
	local min_time, total, oncooldown, available, unavailable =
		ViewManager:GetAbilityStats(cell.frame.view, cell.ability)
...
function mod:InitializeSpellBarInstance(bar, instance)
	...
	local texture = UI:MediaFetch("statusbar", profile.barTexture)
...
function mod:UpdateSpellBarFont(bar)
	local profile = bar.cell.frame.profile
	local font = UI:MediaFetch("font", profile.barFont)
...
function mod:UpdateNameplateVisuals(bar, ismerged)
	local profile = bar.cell.frame.profile
	local font = UI:MediaFetch("font", profile.npFont)
	...
	local texture = UI:MediaFetch("statusbar", profile.npTexture)
	LibBars:SetBackgroundTexture(bar, texture)
	LibBars:SetForegroundTexture(bar, texture)
	...
	if not ismerged then
		...
		LibBars:Draw(bar, npwidth, npheight, profile.npIcon, profile.npTextSide, 100, "right", "empty", "right", "empty")
	else
		if profile.npShowLabel == true then
		...
		LibBars:SetIconTexture(bar, "")
		LibBars:Draw(bar, npwidth, npheight, "none", profile.npTextSide, 100, "right", "empty", "right", "empty")
	end
end
...
function mod:SetSpellBarStyleAvailable(bar)
	...
	if profile.barCCA == true then
		...
		LibBars:SetForegroundTextureColor(bar, color.r, color.g, color.b, profile.barColorA.a)
	else
		LibBars:SetForegroundTextureColor(bar, profile.barColorA.r, profile.barColorA.g, profile.barColorA.b, profile.barColorA.a)
	end

	if profile.barCCAFont == true then
		local color = Hermes:GetClassColorRGB(bar.instance.sender.class)
		LibBars:SetFontColorText(bar, color.r, color.g, color.b, profile.barColorAFont.a)
	else
		LibBars:SetFontColorText(bar, profile.barColorAFont.r, profile.barColorAFont.g, profile.barColorAFont.b, profile.barColorAFont.a)
	end
...
function mod:SetSpellBarStyleOnCooldown(bar)
	...
	if profile.barBGCCC == true then
		local color = Hermes:GetClassColorRGB(bar.instance.sender.class)
		LibBars:SetBackgroundTextureColor(bar, color.r, color.g, color.b, profile.barBGColorC.a)
	else
		LibBars:SetBackgroundTextureColor(bar, profile.barBGColorC.r, profile.barBGColorC.g, profile.barBGColorC.b, profile.barBGColorC.a)
	end

	if profile.barCCC == true then
		local color = Hermes:GetClassColorRGB(bar.instance.sender.class)
		LibBars:SetForegroundTextureColor(bar, color.r, color.g, color.b, profile.barColorC.a)
	else
		LibBars:SetForegroundTextureColor(bar, profile.barColorC.r, profile.barColorC.g, profile.barColorC.b, profile.barColorC.a)
	end

	if profile.barCCCFont == true then
		local color = Hermes:GetClassColorRGB(bar.instance.sender.class)
		LibBars:SetFontColorText(bar, color.r, color.g, color.b, profile.barColorCFont.a)
		LibBars:SetFontColorDuration(bar, color.r, color.g, color.b, profile.barColorCFont.a)
	else
		LibBars:SetFontColorText(bar, profile.barColorCFont.r, profile.barColorCFont.g, profile.barColorCFont.b, profile.barColorCFont.a)
		LibBars:SetFontColorDuration(bar, profile.barColorCFont.r, profile.barColorCFont.g, profile.barColorCFont.b, profile.barColorCFont.a)
	end
...
function mod:SetSpellBarStyleUnavailable(bar)
	...
	if profile.barBGCCU == true then
		local color = Hermes:GetClassColorRGB(bar.instance.sender.class)
		LibBars:SetBackgroundTextureColor(bar, color.r, color.g, color.b, profile.barBGColorU.a)
	else
		LibBars:SetBackgroundTextureColor(bar, profile.barBGColorU.r, profile.barBGColorU.g, profile.barBGColorU.b, profile.barBGColorU.a)
	end

	if profile.barCCU == true then
		local color = Hermes:GetClassColorRGB(bar.instance.sender.class)
		LibBars:SetForegroundTextureColor(bar, color.r, color.g, color.b, profile.barColorU.a)
	else
		LibBars:SetForegroundTextureColor(bar, profile.barColorU.r, profile.barColorU.g, profile.barColorU.b, profile.barColorU.a)
	end

	if profile.barCCUFont == true then
		local color = Hermes:GetClassColorRGB(bar.instance.sender.class)
		LibBars:SetFontColorText(bar, color.r, color.g, color.b, profile.barColorUFont.a)
		LibBars:SetFontColorDuration(bar, color.r, color.g, color.b, profile.barColorUFont.a)
	else
		LibBars:SetFontColorText(bar, profile.barColorUFont.r, profile.barColorUFont.g, profile.barColorUFont.b, profile.barColorUFont.a)
		LibBars:SetFontColorDuration(bar, profile.barColorUFont.r, profile.barColorUFont.g, profile.barColorUFont.b, profile.barColorUFont.a)
	end
		...
		celloptions = {
			...
			args = {
				backgroundcolor = {
					...
					width = "normal",
					get = function(info)
						return profile.cellBGColor.r, profile.cellBGColor.g, profile.cellBGColor.b, profile.cellBGColor.a
					end,
					set = function(info, r, g, b, a)
						...
						for _, cell in ipairs(frame.cells) do
							cell.backgroundTexture:SetTexture(profile.cellBGColor.r, profile.cellBGColor.g, profile.cellBGColor.b, profile.cellBGColor.a)
						end

						--update the mergedcell
						frame.mergedcell.backgroundTexture:SetTexture(profile.cellBGColor.r, profile.cellBGColor.g, profile.cellBGColor.b, profile.cellBGColor.a)
					end
				}
			}
		},
						...
						AvailableBar = {
							...
							args = {
								barColorA = {
									...
									width = "normal",
									get = function(info)
										return profile.barColorA.r, profile.barColorA.g, profile.barColorA.b, profile.barColorA.a
									end,
									set = function(info, r, g, b, a)
						...
						OnCooldownBar = {
							...
							args = {
								barColorC = {
									...
									width = "normal",
									disabled = function()
										return profile.cellHideNoAvailSender
									end,
									get = function(info)
										return profile.barColorC.r, profile.barColorC.g, profile.barColorC.b, profile.barColorC.a
									end,
									set = function(info, r, g, b, a)
								...
								barBGColorC = {
									...
									width = "normal",
									get = function(info)
										return profile.barBGColorC.r, profile.barBGColorC.g, profile.barBGColorC.b, profile.barBGColorC.a
									end,
									set = function(info, r, g, b, a)
						...
						UnavialableBar = {
							...
							args = {
								barColorU = {
									...
									width = "normal",
									get = function(info)
										return profile.barColorU.r, profile.barColorU.g, profile.barColorU.b, profile.barColorU.a
									end,
									set = function(info, r, g, b, a)
								...
								barBGColorU = {
									...
									width = "normal",
									get = function(info)
										return profile.barBGColorU.r, profile.barBGColorU.g, profile.barBGColorU.b, profile.barBGColorU.a
									end,
									set = function(info, r, g, b, a)
						...
						AvailableFont = {
							...
							args = {
								barColorAFont = {
									...
									width = "normal",
									get = function(info)
										return profile.barColorAFont.r, profile.barColorAFont.g, profile.barColorAFont.b, profile.barColorAFont.a
									end,
									set = function(info, r, g, b, a)
						...
						OnCooldownFont = {
							...
							args = {
								barColorCFont = {
									...
									width = "normal",
									get = function(info)
										return profile.barColorCFont.r, profile.barColorCFont.g, profile.barColorCFont.b, profile.barColorCFont.a
									end,
									set = function(info, r, g, b, a)
						...
						UnavailableFont = {
							...
							args = {
								barColorUFont = {
									...
									get = function(info)
										return profile.barColorUFont.r, profile.barColorUFont.g, profile.barColorUFont.b, profile.barColorUFont.a
									end,
									set = function(info, r, g, b, a)
			...
			args = {
				layout = {
					...
					args = {
						...
						npTextSide = {
							...
							width = "full",
							get = function(info)
								return profile.npTextSide == "right"
							end,
							order = 15,
				...
				size = {
					...
					disabled = function(info)
						return profile.npUseNameplate == false
					end,
					args = {
						npW = {
							type = "range",
						...
						BarColor = {
							...
							args = {
								npTexColor = {
									...
									width = "normal",
									get = function(info)
										return profile.npTexColor.r, profile.npTexColor.g, profile.npTexColor.b, profile.npTexColor.a
									end,
									set = function(info, r, g, b, a)
						...
						FontColor = {
							...
							args = {
								FontColor = {
									...
									width = "normal",
									get = function(info)
										return profile.npFontColor.r, profile.npFontColor.g, profile.npFontColor.b, profile.npFontColor.a
									end,
									set = function(info, r, g, b, a)
		...
		oneshottimer = {
			...
			args = {
				...
				osCooldownDirection = {
					...
					width = "normal",
					get = function(info)
						return profile.osCooldownDirection == "left"
					end,
					order = 25,
					disabled = profile.osEnabled ~= true,
					set = function(info, value)
				...
				osFGColor = {
					...
					disabled = profile.osEnabled ~= true,
					get = function(info)
						return profile.osFGColor.r, profile.osFGColor.g, profile.osFGColor.b, profile.osFGColor.a
					end,
					set = function(info, r, g, b, a)
...
function mod:OnLibSharedMediaUpdate(view, mediatype, key) --OPTIONAL
	--update all the cells in the container
	if view.frame and (mediatype == "statusbar" or mediatype == "font") then
		for _, cell in ipairs(view.frame.cells) do
			self:UpdateNameplateVisuals(cell.nameplate)
			for _, bar in ipairs(cell.bars) do
				self:SetSpellBarStyle(bar)
			end
		...
		--handle merged cell
		self:UpdateNameplateVisuals(view.frame.mergedcell.nameplate, 1)
		for _, bar in ipairs(view.frame.mergedcell.bars) do
			self:SetSpellBarStyle(bar)
		end
	end
end
...
function mod:OnInstanceStartCooldown(view, ability, instance) --OPTIONAL
	...
	if frame.profile.osEnabled == true then
		...
		local metadata =
			Hermes:GetAbilityMetaDataValue(ability.id, "duration") or
			Hermes:GetAbilityMetaDataValue(ability.id, "2ndcooldown") --legacy treck support
