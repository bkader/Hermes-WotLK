...
-- catch when an instance going on cooldown should
-- animate using 2ndcooldown metadata or not.
local NEW_INSTANCE_THRESHOLD = 1
...
--sets the starting point for all window positions
-- based on profile settings (if it exists)
function mod:RestoreFramePosition(frame)
...
function mod:SetFramePosRelativeToAnchor(frame, x, y)
	-- this is a matter of setting the window
	-- topleft corner relative to the anchor
	local profile = frame.profile
	-- just set x and y
	profile.x = x
	profile.y = y
...
function mod:CreateCell()
	...
	-- this is a precautionary measure to HOPEFULLY avoid any frames stuck
	-- to cursor issues while dragging if the frame gets hidden while
	-- dragging it (due to combat events)
	cell:SetScript("OnHide", function() cell:StopMovingOrSizing() end)
...
function mod:InitializeSpellBarInstance(bar, instance)
	...
	-- we need this to know when the bar was created relative to
	-- the instance in order to determine whether to fire a
	-- secondary animation for instances on cooldown
	bar.created = GetTime()
...
function mod:OnInstanceStartCooldown(view, ability, instance) --OPTIONAL
	...
	if frame.profile.osEnabled == true then
		...
		local metadata = Hermes:GetAbilityMetaDataValue(ability.id, "duration")
		metadata = metadata or Hermes:GetAbilityMetaDataValue(ability.id, "2ndcooldown") --legacy treck support
