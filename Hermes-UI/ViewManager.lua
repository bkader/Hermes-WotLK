...
if not ACD then
	error("Required library AceConfigDialog-3.0 not found")
end

local LSM = LibStub("LibSharedMedia-3.0")
if not LSM then
	error("Required library LibSharedMedia-3.0 not found")
end
...
function mod:OnInitialize()
	...
	LSM.RegisterCallback(self, "LibSharedMedia_Registered", "OnLibSharedMediaUpdate") --register for media update callbacks
end
		...
		if player.name == UnitName("player") then
			...
			for i = 1, members do
				...
				if player then
					...
					for sender, p in pairs(HERMES_SENDERS) do
						if p.name == name then
							p.unit, p.group = unit, nil
							 --always set group to nil when in a party
							break --stop looping through senders
						end
					end
				end
...
function mod:InsertHermesSender(sender)
	...
	for _, view in ipairs(VIEWS) do
		...
		if not VIEW_SENDERS[view] then
			error("nil view players table")
		end
...
function mod:SyncViewWithReceivingState(view)
	...
	for id, ability in pairs(HERMES_ABILITIES) do
		if view.instances[ability] then
			error("error")
		end --error checking
		local profile = VIEW_PROFILES[view]
		local info, _ = self:GetViewAbilityInfo(profile, ability.id)
...
function mod:SortViewAbilities(view)
	local profile = VIEW_PROFILES[view]
	sort(view.abilities, function(a, b)
...
function mod:SortViewAbilityInstances(view, ability, includeall, fireevent) --sorts the instances for the specified ability
	...
	if includeall then
		sort(view.instances["all"], function(a, b)
			return SortInstancesByAvailability(view, a, b)
		end)
	end

	if ability then
		sort(view.instances[ability], function(a, b)
			return SortInstancesByAvailability(view, a, b)
		end)
		if fireevent then
...
function mod:CheckLastSelectedView()
	...
	if not SELECTED_VIEW then
		...
		if #dbp.views > 0 then
			SELECTED_VIEW = {profile = dbp.views[1]}
...
function mod:UpdateViewSpellsTable()
	...
	if not SELECTED_VIEW or not SELECTED_VIEW.view then
		...
		blizzArgs.viewspells.args = {empty = EMPTY_ARG}
		return
	end
			...
			blizzArgs.viewspells.args[tostring(id)] = {
				...
				args = {
					name = {
						...
						--disabled = true,
						get = function()
							return abilityProfile.enabled or profile.includeAll == true
						end,
						set = function(info, value)
							if (value) then
	...
	blizzArgs.playerfilters.args.names = {
		...
		args = {
			filter = {
				...
				order = 5,
				get = function()
					return profile.filterplayertype
				end,
				values = {
					["disabled"] = L["Disabled"],
					["white"] = L["Whitelist"],
...
function mod:UpdateViewGeneralTable()
	...
	if not SELECTED_VIEW or not SELECTED_VIEW.view then
		...
		blizzArgs.viewgeneral.args = {empty = EMPTY_ARG}
	...
	blizzArgs.viewgeneral.args = {
		name = {
			...
			set = function(info, value)
				...
				ACD:SelectGroup(Hermes.HERMES_VERSION_STRING, "UI", "viewgeneral", profile.name)
			end
		},
		spacer1 = {
			type = "description",
...
function mod:UpdateViewModuleTable()
	...
	if not SELECTED_VIEW or not SELECTED_VIEW.view then
		...
		blizzArgs.viewmodule.args = {empty = EMPTY_ARG}
...
function mod:SortProfileViews()
	sort(dbp.views, function(a, b) return a.name < b.name end)
end
	...
	blizzArgs = {
		...
		enabled = {
			...
			get = function()
				...
				if SELECTED_VIEW then
					if SELECTED_VIEW.profile == nil then
						error("error")
					end -- error checking
			...
			set = function(info, value)
				...
				if value == true then
					...
					if SELECTED_VIEW.view ~= nil then
						error("error")
					end --error checking
					if SELECTED_VIEW.profile == nil then
						error("error")
					end --error checking
					if SELECTED_VIEW.profile.enabled ~= false then
						error("error")
					end --error checking
		...
		delete = {
			...
			order = 15,
			confirm = function()
				return L["Container and it's settings will be deleted permanently. Continue?"]
			end,
			disabled = SELECTED_VIEW == nil,
			func = function()
				if SELECTED_VIEW == nil then
					error("error")
				end --error checking
				if SELECTED_VIEW.profile == nil then
					error("error")
